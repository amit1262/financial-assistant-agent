...
def process_query(user_id: str, user_query: str) -> dict:
    """Process the user query through the agent workflow and return the result with status signal

    Args:
        user_id: Identifier for user (used as thread_id for persistence)
        user_query: The user's question

    Returns:
        dict with keys:
            - answer: Agent's response string (None if failed)
            - status: "success", "empty_response", or "error"
            - error_message: Error details if applicable (None if success)
    """
