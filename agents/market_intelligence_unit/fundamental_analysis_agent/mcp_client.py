...
class MCPClient:
    ...
    async def _create_multi_server_mcp_client(
        ...
        if not servers:
            logger.warning(
                "[Fundamental Agent MCPClient] No servers available to create MCP client"
            )
            raise ValueError(
                "[Fundamental Agent MCPClient] No servers available to create MCP client"
            )
        try:
            ...
            logger.info("[Fundamental Agent MCPClient] MCP Client successfully created")
            return client
        except Exception as e:
            logger.error(
                f"[Fundamental Agent MCPClient] Error creating MCP Client: {e}"
            )
            raise ValueError(
                f"[Fundamental Agent MCPClient] Error creating MCP Client: {e}"
            )
    ...
    async def _get_servers_from_config(self, mcp_config_path) -> dict:
        ...
        try:
            ...
            if not servers_dict:
                logger.warning(
                    "[Fundamental Agent MCPClient] No servers configured in mcp-config.json"
                )
                raise ValueError(
                    "[Fundamental Agent MCPClient] No servers configured in mcp-config.json"
                )
            ...
            logger.error(f"[Fundamental Agent MCPClient] Config file missing: {e}")
            raise ValueError(f"[Fundamental Agent MCPClient] Config file missing: {e}")

        except json.JSONDecodeError as e:
            logger.error(f"[Fundamental Agent MCPClient] Invalid JSON in config: {e}")
            raise ValueError(
                f"[Fundamental Agent MCPClient] Invalid JSON in config: {e}"
            )
    ...
    async def fetch_mcp_tools(self) -> None:
        try:
            if not self.mcp_client:
                logger.warning(
                    "[Fundamental Agent MCPClient] MCP client not initialized, cannot fetch tools"
                )
                raise ValueError(
                    "[Fundamental Agent MCPClient] MCP client not initialized, cannot fetch tools"
                )
            ...
            logger.info(
                f"[Fundamental Agent MCPClient] Fetching tools - MCP client: {self.mcp_client}"
            )
            tools = await self.mcp_client.get_tools(
                server_name=None
            )  # fetch tools from all servers
            logger.info(f"[Fundamental Agent MCPClient] Tools fetched: {len(tools)}")
            self.cached_tools = tools  # cache the tools
        except Exception as e:
            logger.error(f"[Fundamental Agent MCPClient] Error fetching tools: {e}")
    ...
    async def get_mcp_tools(self) -> list:
        ...
        if self.cached_tools is not None:
            logger.info("[Fundamental Agent MCPClient] Returning cached tools")
            return self.cached_tools
        else:
            logger.info(
                "[Fundamental Agent MCPClient] No cached tools, fetching from MCP client"
            )
            await self.fetch_mcp_tools()
            return self.cached_tools
