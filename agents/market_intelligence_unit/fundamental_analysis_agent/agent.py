...
class FundamentalAnalysisAgent:
    ...
    async def _initialize_model(self):
        ...
        model_name = os.getenv("FUNDAMENTAL_MODEL_NAME")
        base_url = os.getenv("BASE_URL")
        api_key = os.getenv("OPENROUTER_API_KEY")
        if model_name is None:
            logger.error("FUNDAMENTAL_MODEL_NAME environment variable is not set")
            raise RuntimeError(
                "FUNDAMENTAL_MODEL_NAME environment variable is required"
            )
        if base_url is None:
