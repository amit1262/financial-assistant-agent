# class that defines and implements fundamental analysis agent for market intelligence unit
...
from fundamental_analysis_agent.mcp_client import MCPClient
...
class FundamentalAnalysisAgent:
    ...
    async def _initialize_model(self):
        ...
        if model_name is None:
            logger.error(
                "FUNDAMENTAL_ANALYSIS_MODEL_NAME environment variable is not set"
            )
            raise RuntimeError(
        ...
        try:
            model = ChatOpenRouter(
                model=model_name,
                temperature=float(
                    os.getenv("FUNDAMENTAL_ANALYSIS_MODEL_TEMPERATURE", 0.2)
                ),
                # max_tokens=1024,
                max_retries=5,
            )
            ...
            logger.info(
                f"[Fundamental Agent] Initialized model: {model_name} with tools: {[tool.name for tool in self.mcp_tools]}"
            )
    ...
    async def _get_mcp_tools(self) -> None:
        ...
        logger.info(
            f"[Fundamental Agent] Fetched MCP tools: {[tool.name for tool in self.mcp_tools]}"
        )
