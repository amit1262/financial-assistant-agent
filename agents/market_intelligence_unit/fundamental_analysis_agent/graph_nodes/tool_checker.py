...
async def tool_checker(state: State, mcp_tools) -> Command:
    ...
    if mcp_tools is None:
        ...
        logger.warning(
            "[Fundamental Agent] No MCP tools available for the agent, ending workflow."
        )
        error_message = "No MCP tools configured/available for the agent."
        # global variables
        fallback_message = (
            "[Fundamental Agent] Failed (No MCP Tools Configured/Available)"
        )
        ...
        logger.info(
            f"[Fundamental Agent] MCP tools available: {len(mcp_tools)}. Continuing workflow."
        )
        update_content = {
            "status": "success",
            "error": "",
