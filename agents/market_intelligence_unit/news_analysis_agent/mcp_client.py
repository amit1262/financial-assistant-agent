...
class MCPClient:
    ...
    async def _create_multi_server_mcp_client(
        ...
        if not servers:
            logger.warning(
                "[News Agent MCPClient] No servers available to create MCP client"
            )
            raise ValueError(
                "[News Agent MCPClient] No servers available to create MCP client"
            )
        try:
            ...
            logger.info("[News Agent MCPClient] MCP Client successfully created")
            return client
        except Exception as e:
            logger.error(f"[News Agent MCPClient] Error creating MCP Client: {e}")
            raise ValueError(f"[News Agent MCPClient] Error creating MCP Client: {e}")
    ...
    async def _get_servers_from_config(self, mcp_config_path) -> dict:
        ...
        try:
            ...
            if not servers_dict:
                logger.warning(
                    "[News Agent MCPClient] No servers configured in mcp-config.json"
                )
                raise ValueError(
                    "[News Agent MCPClient] No servers configured in mcp-config.json"
                )
            ...
            for server_name, server_config in servers_dict.items():
                ...
                if "API_KEY" in url:
                    ...
                    if not api_key:
                        logger.error(
                            f"[News Agent MCPClient] Missing API key for '{server_name}': set {env_key_name} env var"
                        )
                        raise ValueError(
                            f"[News Agent MCPClient] Environment variable not set: {env_key_name}"
                        )
                    else:
            ...
            logger.info(f"[News Agent MCPClient] MCP servers: {len(final_dict)}")
            return final_dict

        except FileNotFoundError as e:
            logger.error(f"[News Agent MCPClient] Config file missing: {e}")
            raise ValueError(f"[News Agent MCPClient] Config file missing: {e}")

        except json.JSONDecodeError as e:
            logger.error(f"[News Agent MCPClient] Invalid JSON in config: {e}")
            raise ValueError(f"[News Agent MCPClient] Invalid JSON in config: {e}")

    async def fetch_mcp_tools(self) -> None:
        try:
            if not self.mcp_client:
                logger.warning(
                    "[News Agent MCPClient] MCP client not initialized, cannot fetch tools"
                )
                raise ValueError(
                    "[News Agent MCPClient] MCP client not initialized, cannot fetch tools"
                )
            ...
            logger.info(
                f"[News Agent MCPClient] Fetching tools - MCP client: {self.mcp_client}"
            )
            tools = await self.mcp_client.get_tools(
                server_name=None
            )  # fetch tools from all servers
            logger.info(f"[News Agent MCPClient] Tools fetched: {len(tools)}")
            self.cached_tools = tools  # cache the tools
        except Exception as e:
            logger.error(f"[News Agent MCPClient] Error fetching tools: {e}")
    ...
    async def get_mcp_tools(self) -> list:
        ...
        if self.cached_tools is not None:
            logger.info("[News Agent MCPClient] Returning cached tools")
            return self.cached_tools
        else:
            logger.info(
                "[News Agent MCPClient] No cached tools, fetching from MCP client"
            )
            await self.fetch_mcp_tools()
            return self.cached_tools
